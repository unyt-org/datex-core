...
use super::{primitives::time::Time, Endpoint, Error, Quantity, Url, Value, ValueResult};
...
impl PrimitiveValue {
        ...
        if self.is_number() && other.is_number() {
            match self {
                ...
                PrimitiveValue::Int16(val) => {
                    Ok(PrimitiveValue::Int16(val + other.get_as_integer() as i16))
                }
                PrimitiveValue::Int32(val) => {
                    Ok(PrimitiveValue::Int32(val + other.get_as_integer() as i32))
                }
                PrimitiveValue::Int64(val) => {
                    Ok(PrimitiveValue::Int64(val + other.get_as_integer() as i64))
                }
    ...
    fn power(&self, other: &PrimitiveValue) -> Result<PrimitiveValue, Error> {
        if self.is_number() && other.is_number() {
            match self {
                PrimitiveValue::Int8(val) => {
                    Ok(PrimitiveValue::Int8(val.pow(other.get_as_integer() as u32)))
                }
