...
impl Quantity {
    ...
    pub fn value_to_string(&self, alias_factor:bool, decimals:Option<u8>) -> String {
        
        let (mut numerator, mut denominator) = (BigInt::from_biguint(Sign::Plus, self.numerator.clone()), BigInt::from_biguint(Sign::Plus, self.denominator.clone()));

        // divide by short_divisor to match alias factor
        if alias_factor {
            (numerator, denominator) = Quantity::normalize_fraction(numerator, denominator*BigInt::from_biguint(Sign::Plus, self.short_divisor.clone()));
        }
        ...
        else if Quantity::has_finite_decimal_rep(&mut denominator.to_biguint().unwrap()) {
    ...
    fn finite_fraction_to_decimal_string(&self, mut numerator:BigInt, denominator:BigInt) -> String {
        ...
        if !Quantity::is_power_of_10(denominator.clone()) {
            ...
            let mut found = false;
            for _ in 0..10000 { // only try 10000 iterations, works in first iteration in most cases
        ...
        let string = (numerator).to_string().pad(
            shift as usize, '0', pad::Alignment::Right, false
        );
        let comma_shift = string.len() - shift as usize;
        let p1 = &string[0..comma_shift];
        let p2 = &string[comma_shift..];

        (if self.sign {""} else {"-"}).to_owned() + p1 + (if p2.len() > 0 {if p1.len() > 0 {"."} else {"0."}} else {""}) + p2
    }
    ...
    fn normalize_fraction<'a>(mut numerator: BigInt, mut denominator: BigInt) -> (BigInt, BigInt) {
