...
impl Quantity {
    ...
    pub fn value_to_string(&self, alias_factor: bool, decimals: Option<u8>) -> String {
        ...
        if alias_factor {
            (numerator, denominator) = Quantity::normalize_fraction(
                numerator,
                denominator * BigInt::from_biguint(Sign::Plus, self.short_divisor.clone()),
            );
        }
    ...
    fn finite_fraction_to_decimal_string(
        ...
        let string = (numerator)
            .to_string()
            .pad(shift as usize, '0', pad::Alignment::Right, false);
    ...
    fn has_same_dimension(&self, other: &Quantity) -> bool {
        ...
        for i in 0..self.unit.len() {
            if self.unit[i].0 != other.unit[i].0 || self.unit[i].1 != other.unit[i].1 {
                return false;
            }
        }
        true
