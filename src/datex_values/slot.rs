...
impl SlotIdentifier {
    ...
    pub fn is_reserved(&self) -> bool {
        self.index >= internal_slot_address_space::RESERVED_OBJECT_SLOTS.0
            && self.index <= internal_slot_address_space::RESERVED.1
    }

    pub fn is_object_slot(&self) -> bool {
        (self.index >= internal_slot_address_space::OBJECT_SLOTS.0
            && self.index <= internal_slot_address_space::OBJECT_SLOTS.1)
            || (self.index
                >= internal_slot_address_space::RESERVED_OBJECT_SLOTS.0
                && self.index
                    <= internal_slot_address_space::RESERVED_OBJECT_SLOTS.1)
    }
}
