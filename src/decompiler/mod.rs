...
fn decompile_loop(state: &mut DecompilerGlobalState) -> String {
    ...
    let instruction_iterator =
        body::iterate_instructions(state.dxb_body, state.index, state.is_end_instruction);
    ...
    for instruction in instruction_iterator {
        ...
        match code {
            ...
            BinaryCode::CHILD_ACTION => {
                out += &get_code_token(&BinaryCode::CHILD_ACTION, state.formatted)
            }
            ...
            BinaryCode::INTERNAL_OBJECT_SLOT => {
                out += &format!(
                    "{}:",
                    SlotIdentifier::new(primitive_value.get_as_unsigned_integer() as u16)
                )
            }
        ...
        if next_assign_action.is_some() {
            ...
            match assign_type {
                ...
                _ => {
                    out += &get_code_token(
                        &BinaryCode::try_from(assign_type).expect("enum conversion error"),
                        false,
                    )
                }
            }
        ...
        match code {
            BinaryCode::CHILD_ACTION => {
                next_assign_action = Some(primitive_value.get_as_integer() as u8)
            }
