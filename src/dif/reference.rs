...
impl DIFReference {
    pub fn from_reference(reference: &Reference, memory: &RefCell<Memory>) -> Self {
        let value = DIFValueContainer::from_value_container(&reference.value_container(), memory);
        let allowed_type = DIFTypeContainer::from_type_container(&reference.allowed_type(), memory);
        DIFReference {
            value,
            allowed_type,
