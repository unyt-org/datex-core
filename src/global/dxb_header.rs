...
impl DXBHeader {
    ...
    pub fn from_bytes<'a>(
        dxb: &'a [u8],
    ) -> Result<DXBHeader, HeaderParsingError> {
        // has magic number?
        if !DXBHeader::has_dxb_magic_number(dxb) {
            return Err(HeaderParsingError::InvalidMagicNumber);
        }
        ...
        let block_type = DXBBlockType::try_from(read_u8(dxb, index))
            .expect("Invalid DXB block type");
    ...
    fn get_dxb_header_sender(
        ...
        if read_u8(dxb, index) == std::u8::MAX {
            ...
            *index -= 1;
            return Some(Endpoint::new_from_binary(&read_slice(
                dxb, index, 21,
            )));
        }
    }

    fn get_dxb_header_receivers(
        dxb: &[u8],
        index: &mut usize,
    ) -> Option<Endpoint> {
    ...
    pub fn pre_header_to_bytes(&self) -> Vec<u8> {
        ...
        if self.routing.sender.is_some() {
            pre_header.extend_from_slice(
                &self.routing.sender.as_ref().unwrap().get_binary(),
            );
        }
        // no sender - anonymous
        else {
