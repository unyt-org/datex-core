...
use crate::datex_values::{
    internal_slot, BaseUnit, Endpoint, Pointer, PrimitiveValue, Quantity, SlotIdentifier, Time,
    Type, Url, Value,
};
...
fn extract_endpoint(dxb_body: &[u8], index: &mut usize, endpoint_type: BinaryCode) -> Endpoint {
    ...
    let name_is_binary =
        endpoint_type == BinaryCode::ENDPOINT || endpoint_type == BinaryCode::ENDPOINT_WILDCARD;
