...
fn extract_type<'a>(
  ...
  if is_extended {
    ...
    _has_parameters = if buffers::read_u8(dxb_body, index) == 0 {
      false
    } else {
      true
    };
...
fn extract_endpoint(
  ...
  if !instance_set {
    let instance_string =
      &buffers::read_string_utf8(dxb_body, index, instance_length as usize); // get instance
      ...
      while _index.get() < max {
        ...
        if token == BinaryCode::INT_8 as u8 {
          ...
          yield Instruction {
            ...
            slot: None,
            primitive_value: Some(PrimitiveValue::Int64(value as i64)),
            value: None,
        ...
        // text
          ...
          let size = buffers::read_u32(&dxb_body, index);
          let value =
            buffers::read_string_utf8(&dxb_body, index, size as usize);
          _index.set(*index);
          yield Instruction {
        ...
        // quantity
          ...
          let num_buffer =
            buffers::read_slice(&dxb_body, index, num_size as usize);
          let den_buffer =
            buffers::read_slice(&dxb_body, index, den_size as usize);
          ...
          yield Instruction {
            ...
            slot: None,
            primitive_value: Some(PrimitiveValue::Quantity(quantity)),
            value: None,
            subscope_continue: false,
          }
        }
        ...
        // slot instructions
          ...
          yield Instruction {
            ...
            slot: Some(slot),
            primitive_value: Some(PrimitiveValue::UInt32(init_block_length)),
            value: None,
            subscope_continue: false,
          }
        ...
        // pointer
          ...
          let id =
            buffers::read_slice(&dxb_body, index, Pointer::MAX_POINTER_ID_SIZE);
        ...
        // endpoints
          ...
          let endpoint =
            extract_endpoint(&dxb_body, index, BinaryCode::PERSON_ALIAS);
          _index.set(*index);
          yield Instruction {
            code: BinaryCode::PERSON_ALIAS,
            slot: None,
            primitive_value: Some(PrimitiveValue::Endpoint(endpoint)),
            value: None,
            subscope_continue: false,
          }
        } else if token == BinaryCode::INSTITUTION_ALIAS as u8 {
          let endpoint =
            extract_endpoint(&dxb_body, index, BinaryCode::INSTITUTION_ALIAS);
          _index.set(*index);
          yield Instruction {
          ...
          let endpoint =
            extract_endpoint(&dxb_body, index, BinaryCode::ENDPOINT);
          _index.set(*index);
          yield Instruction {
