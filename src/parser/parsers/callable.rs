...
use crate::ast::structs::expression::{DatexExpression, CallableDeclaration, CallableKind};
use crate::ast::structs::r#type::TypeExpression;
use crate::parser::{SpannedParserError, Parser};

impl Parser {
    pub(crate) fn parse_callable_definition(&mut self) -> Result<DatexExpression, SpannedParserError> {
        ...
        let return_type = if let Ok(token) = self.peek() && token.token == Token::Arrow {
            self.advance()?;
    ...
    fn parse_callable_parameters(&mut self) -> Result<Vec<(String, TypeExpression)>, SpannedParserError> {
...
mod tests {
    ...
    use crate::ast::{DatexExpressionData, Statements};
    use crate::ast::spanned::Spanned;
    use crate::ast::structs::expression::{BinaryOperation, CallableDeclaration, CallableKind};
    ...
    fn parse_empty_function() {
        ...
        assert_eq!(expr.data, DatexExpressionData::CallableDeclaration(
            CallableDeclaration {
                name: "test".to_string(),
                kind: CallableKind::Function,
                parameters: vec![],
                return_type: None,
                body: Box::new(DatexExpressionData::Statements(
                    Statements {
                        statements: vec![],
                        is_terminated: false,
                        unbounded: None,
    ...
    fn parse_function_with_parameters_and_return_type() {
        ...
        assert_eq!(expr.data, DatexExpressionData::CallableDeclaration(
            CallableDeclaration {
                ...
                kind: CallableKind::Function,
                parameters: vec![
                    ("a".to_string(), TypeExpressionData::Identifier("integer".to_string()).with_default_span()),
                    ("b".to_string(), TypeExpressionData::Identifier("integer".to_string()).with_default_span()),
                ],
                return_type: Some(TypeExpressionData::Identifier("integer".to_string()).with_default_span()),
                body: Box::new(DatexExpressionData::Statements(
                    Statements {
                        statements: vec![],
                        is_terminated: false,
    ...
    fn parse_function_with_parameters_and_body() {
        let expr = parse("function greet(name: text) -> text ( \"Hello, \" + name )");
        assert_eq!(expr.data, DatexExpressionData::CallableDeclaration(
            CallableDeclaration {
                name: "greet".to_string(),
                kind: CallableKind::Function,
                parameters: vec![
                    ("name".to_string(), TypeExpressionData::Identifier("text".to_string()).with_default_span()),
                ],
                return_type: Some(TypeExpressionData::Identifier("text".to_string()).with_default_span()),
                body: Box::new(
                    DatexExpressionData::BinaryOperation(BinaryOperation {
                        left: Box::new(DatexExpressionData::Text("Hello, ".to_string()).with_default_span()),
                        operator: BinaryOperator::Arithmetic(ArithmeticOperator::Add),
                        right: Box::new(DatexExpressionData::Identifier("name".to_string()).with_default_span()),
                        ty: None,
                    }).with_default_span(),
                )
            }
