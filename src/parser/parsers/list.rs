...
impl Parser {
    pub fn parse_list(&mut self) -> Result<DatexExpression, SpannedParserError> {
        ...
        while self.peek()?.token != Token::RightBracket {
            ...
            let expression = self.recover_on_error(maybe_expression, &[Token::Comma, Token::RightBracket])?;
...
mod tests {
    ...
    use crate::parser::tests::{parse, try_parse_and_collect_errors, try_parse_and_return_on_first_error};
    ...
    fn parse_recover_from_error() {
        ...
        if let ParserResult::Invalid(res) = res {
            ...
            assert_eq!(res.ast.data, DatexExpressionData::List(List {
                items: vec![
                    DatexExpressionData::Boolean(true).with_default_span(),
