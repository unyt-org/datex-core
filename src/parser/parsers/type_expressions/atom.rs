...
impl Parser {
    pub(crate) fn parse_type_atom(
        ...
        Ok(match self.peek()?.token.clone() {
            ...
            Token::Endpoint(endpoint_name) => {
                self.parse_type_endpoint(endpoint_name)?
            }
            Token::IntegerLiteral(integer_literal) => {
                self.parse_type_integer_literal(integer_literal)?
            }
    ...
    pub(crate) fn parse_type_string_literal(
        &mut self,
        value: String,
    ) -> Result<TypeExpression, SpannedParserError> {
    ...
    pub(crate) fn parse_type_infinity(
        &mut self,
    ) -> Result<TypeExpression, SpannedParserError> {
        Ok(TypeExpressionData::Decimal(Decimal::Infinity)
            .with_span(self.advance()?.span))
    }

    pub(crate) fn parse_type_nan(
        &mut self,
    ) -> Result<TypeExpression, SpannedParserError> {
        Ok(TypeExpressionData::Decimal(Decimal::Nan)
            .with_span(self.advance()?.span))
    }

    pub(crate) fn parse_type_endpoint(
        &mut self,
        ...
        match Endpoint::from_str(endpoint_name.as_str()) {
            ...
            Ok(endpoint) => {
                Ok(TypeExpressionData::Endpoint(endpoint).with_span(span))
            }
        }
    }

    pub(crate) fn parse_type_integer_literal(
        &mut self,
        ...
        Ok(match parse_integer_literal(integer_literal) {
            ...
            Err(e) => {
                return self.collect_error_and_continue_with_type_expression(
                    SpannedParserError {
    ...
    pub(crate) fn parse_type_integer_with_variant(
        &mut self,
        ...
        Ok(
            match parse_integer_with_variant(integer_with_variant, token.token)
            {
                ...
                Err(e) => {
                    return self
                        .collect_error_and_continue_with_type_expression(
                            SpannedParserError {
                                error: ParserError::NumberParseError(e),
                                span,
                            },
    ...
    pub(crate) fn parse_type_decimal_literal(
        &mut self,
        ...
        match res {
            ...
            Err(e) => self.collect_error_and_continue_with_type_expression(
                SpannedParserError {
                    error: ParserError::NumberParseError(e),
                    span,
                },
    ...
    pub(crate) fn parse_type_fraction_literal(
        &mut self,
        fraction: String,
    ) -> Result<TypeExpression, SpannedParserError> {
        let span = self.advance()?.span.clone();
