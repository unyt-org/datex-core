...
impl Parser {
    pub fn parse_type_map(&mut self) -> Result<TypeExpression, SpannedParserError> {
        ...
        Ok(
            TypeExpressionData::StructuralMap(StructuralMap(entries)).with_span(start..end)
        )
    }
}
...
mod tests {
    ...
    fn parse_simple_map() {
        ...
        assert_eq!(expr.data, TypeExpressionData::StructuralMap(StructuralMap(vec![
            (
                TypeExpressionData::Text("key1".to_string()).with_default_span(),
                TypeExpressionData::Boolean(true).with_default_span()
            ),
            (
                TypeExpressionData::Text("key2".to_string()).with_default_span(),
    ...
    fn parse_map_with_dynamic_expression_keys() {
        ...
        assert_eq!(expr.data, TypeExpressionData::StructuralMap(StructuralMap(vec![
            (
                TypeExpressionData::Identifier("x".to_string()).with_default_span(),
                TypeExpressionData::Boolean(true).with_default_span()
            ),
            (
                TypeExpressionData::Intersection(Intersection(
                    vec![
                        TypeExpressionData::Identifier("y".to_string()).with_default_span(),
                        TypeExpressionData::Boolean(true).with_default_span(),
                    ]
                )).with_default_span(),
                TypeExpressionData::Boolean(false).with_default_span(),
