...
pub struct Precompiler<'a> {
    options: PrecompilerOptions,
    spans: Option<&'a Vec<Range<usize>>>,
    metadata: Option<AstMetadata>,
    scope_stack: PrecompilerScopeStack,
    error: PrecompilerError,
}
...
pub enum PrecompilerError {
    ...
    Collection(DetailedCompilerErrors),
    /// SimpleCompilerErrors
    Single(Option<SpannedCompilerError>),
}
impl PrecompilerError {
    fn new_detailed() -> Self {
        PrecompilerError::Collection(DetailedCompilerErrors::default())
    }
    fn new_simple() -> Self {
        PrecompilerError::Single(None)
    }
    fn has_errors(&self) -> bool {
        match self {
            PrecompilerError::Collection(errors) => errors.has_errors(),
            PrecompilerError::Single(Some(_)) => true,
            PrecompilerError::Single(None) => false,
        }
    }
    fn record_error(&mut self, error: SpannedCompilerError) {
        match self {
            PrecompilerError::Collection(errors) => {
                errors.record_error(error);
            }
            PrecompilerError::Single(slot) => {
                *slot = Some(error);
            }
        }
    }
...
impl<'a> Precompiler<'a> {
    pub fn new(options: PrecompilerOptions) -> Self {
        let error = if options.detailed_errors {
            PrecompilerError::new_detailed()
        } else {
            PrecompilerError::new_simple()
        };
        Self {
            options,
            spans: None,
    ...
    pub fn precompile(
        &mut self,
        ast: &'a mut ValidDatexParseResult,
        ...
        self.spans = Some(&ast.spans);
        self.error = if self.options.detailed_errors {
            PrecompilerError::new_detailed()
        } else {
            PrecompilerError::new_simple()
        };
        ...
        //     if let Some(collected_errors) = self.error.as_mut()
        ...
        // if let Some(errors) = self.error.take()
        //     && errors.has_errors()
        // {
        //     Err(
    ...
    fn span(&self, span: &Range<usize>) -> Option<Range<usize>> {
        ...
        if span.start != 0 || span.end != 0 {
            let start_token =
                self.spans.unwrap().get(span.start).cloned().unwrap();
            let end_token =
                self.spans.unwrap().get(span.end - 1).cloned().unwrap();
...
// impl<'a> TypeExpressionVisitor for Precompiler<'a> {}
// impl<'a> ExpressionVisitor for Precompiler<'a> {
...
//         });
//         let action = collect_or_pass_error(&mut self.error, result).unwrap(); // FIXME: handle error properly
...
//                 });
//             let action =
//                 collect_or_pass_error(&mut self.error, resolved_variable)
//                     .unwrap(); // FIXME: handle error properly
//             if let MaybeAction::Do(resolved_variable) = action {
