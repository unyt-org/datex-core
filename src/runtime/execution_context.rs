...
impl ExecutionContext {
    ...
    fn set_compile_scope(&mut self, new_compile_scope: CompileScope) {
        match self {
            ExecutionContext::Local { compile_scope, .. } => *compile_scope = new_compile_scope,
            ExecutionContext::Remote { compile_scope, .. } => *compile_scope = new_compile_scope,
        }
    }
    ...
    pub fn execute_dxb_local(&mut self, dxb: &[u8]) -> Result<Option<ValueContainer>, ExecutionError> {
        let (local_execution_context, execution_options, verbose) = match self {
            ExecutionContext::Local { local_execution_context, execution_options, verbose, .. } => (local_execution_context, execution_options, *verbose),
            ExecutionContext::Remote { .. } => {
        ...
        if verbose {
            println!("\x1b[32m[Compiled Bytecode] {}", dxb.iter().map(|b| format!("{:02x}", b)).collect::<Vec<_>>().join(", "));
    ...
    pub fn execute_local(&mut self, script: &str, inserted_values: &[ValueContainer]) -> Result<Option<ValueContainer>, ScriptExecutionError> {
        let dxb = self.compile(script, inserted_values)?;
        self.execute_dxb_local(&dxb).map_err(ScriptExecutionError::ExecutionError)
    }
}