...
impl StructuralEq for NominalTypeDeclaration {
    fn structural_eq(&self, other: &Self) -> bool {
        self.name == other.name && self.variant == other.variant && self.definition.structural_eq(&other.definition)
    }
}
...
impl StructuralEq for TypeDefinition {
    fn structural_eq(&self, other: &Self) -> bool {
        match (self, other) {
            (TypeDefinition::Structural(a), TypeDefinition::Structural(b)) => a.structural_eq(b),
            (TypeDefinition::Nominal(a), TypeDefinition::Nominal(b)) => a.structural_eq(b),
...
impl Type {
    ...
    pub fn get_base_type(&self) -> Type {
        match &self.type_definition {
            ...
            TypeDefinition::Union(types) => {
                let base_types: Vec<Type> = types.iter().map(|t| t.get_base_type()).collect();
                Type {
    ...
    pub fn value_matches_type(value: &ValueContainer, match_type: &Type) -> bool {
        match match_type {
            // e.g. 1 matches 1 | 2
            Type { type_definition: TypeDefinition::Union(types), .. } => {
                // value must match at least one of the union types
                types.iter().any(|t| Type::value_matches_type(value, t))
            }
            Type { type_definition: TypeDefinition::Structural(type_value_container), .. } => {
                ...
                match type_value {
                    ...
                    | CoreValue::Endpoint(_)  => {
                        type_value.structural_eq(&value.to_value().borrow().inner)
                    }
                }
            }
            Type { type_definition: TypeDefinition::Nominal(NominalTypeDeclaration { definition, .. }), .. } => {
                // compare nominal type directly (e.g. 1 matches integer)
                // TODO: also check if base type matches
                if let TypeDefinition::Nominal(NominalTypeDeclaration { definition: val_type, .. }) = value.actual_type().type_definition {
...
impl StructuralEq for Type {
    fn structural_eq(&self, other: &Self) -> bool {
        self.type_definition.structural_eq(&other.type_definition) && self.reference_mutability == other.reference_mutability
    }
}
...
impl Display for Type {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let mutability = self.reference_mutability.as_ref().map_or("".to_string(), |m| m.to_string());
...
mod tests {
    ...
    fn test_match_equal_values() {
        // 1 matches 1
        assert!(
            Type::value_matches_type(
                &ValueContainer::from(1),
                &Type {
                    type_definition: TypeDefinition::Structural(Box::new(ValueContainer::from(1))),
                    reference_mutability: None,
                }
            )
        )
    ...
    fn test_match_union() {
        ...
        assert!(
            Type::value_matches_type(
                ...
                &Type {
                    type_definition: TypeDefinition::Union(vec![
                        Type {
                            type_definition: TypeDefinition::Structural(Box::new(ValueContainer::from(Integer::from(1)))),
                            reference_mutability: None,
                        },
                        Type {
                            type_definition: TypeDefinition::Structural(Box::new(ValueContainer::from(Integer::from(2)))),
                            reference_mutability: None,
                        },
                        Type {
                            type_definition: TypeDefinition::Structural(Box::new(ValueContainer::from(Integer::from(3)))),
                            reference_mutability: None,
                        },
                    ]),
                    reference_mutability: None,
