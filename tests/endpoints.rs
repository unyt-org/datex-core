...
pub fn create_endpoints() {
    ...
    assert_eq!(
        buffer_to_hex_advanced(
            person_endpoint.get_binary().to_vec(),
            " ",
            0,
            false
        ),
...
pub fn endpoints_from_binary() {
    ...
    // create new
    let cloned_id_endpoint =
        Endpoint::new_from_binary(id_endpoint.get_binary());
    assert_eq!(
        id_endpoint.to_string(false),
    ...
    // create new
    let cloned_person_endpoint =
        Endpoint::new_from_binary(person_endpoint.get_binary());
    assert_eq!(
