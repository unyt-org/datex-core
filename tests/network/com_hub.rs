...
pub async fn test_add_and_remove() {
    init_global_context();
    let com_hub = Rc::new(ComHub::default_with_async_context(AsyncContext::new()));
    let uuid = {
...
pub async fn test_multiple_add() {
    ...
    let com_hub = ComHub::default_with_async_context(AsyncContext::new());
...
pub async fn register_factory() {
    run_async! {
        init_global_context();
        let mut com_hub = ComHub::default_with_async_context(AsyncContext::new());
