...
use datex_core::global::{
    dxb_block::DXBBlock,
    protocol_structures::{
        addressing::{Endpoint, EndpointType, Sender},
...
pub fn parse_encrypted_header() {
    ...
    assert!(header_result.to_bytes().unwrap() == encrypted_header.to_bytes().unwrap());
}
...
pub fn parse_block_header() {
    ...
    assert!(header_result.to_bytes().unwrap() == block_header.to_bytes().unwrap());
}
...
pub fn parse_routing_header() {
    ...
    assert!(header_result.to_bytes().unwrap() == routing_header.to_bytes().unwrap());
    // }
}
